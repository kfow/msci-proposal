...
One aspect of this review process is the comparison of document contents to information in the public domain, suc::h as news articles.
...
This approach to generating a test collection would not be a suitable substitution in other sensitivity review scenarios, but we are not considering document \textbf{sentiment} here, but rather the objective terms and phrases mentioned in a document, which are similar across source and target documents in practice (e.g. named entities, temporal references and noun phrase chunks).
...
% mention that we wish to complete this in two stages
...
These experiments directly correspond to the hypotheses of Table~\ref{hypotheses.table} and are ordered from the simplest to the most complex. Table~\cite{NEEDED} shows these hypotheses and also make clear which part of the system they intend to evaluate.
\todo{give label}
